...
import { fetchingJobInfo, fetchingJobs, getCreatorJobs, getEmployerJobs, Jobsforyou, likeJob, likeJobInterface, saveJob } from "../Services/jobs.service";
import { creatorJobsNotFound, errorFetchingJob404, errorFetchingJobs, JobAdError, JobFetchErrorServer, LikedServerErrorMessage, NotLoggedInErrorMessage, SavedJobServerErrorMessage, serverErrorOccurredCreatorJobs } from "../ErrorMessages/errorMessages";
import { toast } from "sonner"; // or wherever your toast comes from
import { fetchJobAds } from "../Services/ads.service";
import { fetchCreatorJobsInterface, fetchEmployerDashboardJobsProps, fetchEmployerDashboardJobsReturn, fetchJobsForYouInterface, getJobInfoInterface, likeJobHookInterface, saveJobHookInterface } from "../interfaces/JobInterfaces";
...
interface useJobsInterface {
    ...
    fetchJobsForYou:(props:fetchJobsForYouInterface)=>Promise<void>
}
   ...
   if(response.status==200){
    ...
    const getJobInfo =async({
        ...
        setIsProposalSent,
         ...
         console.log('jkobinfo response',response)  
         const result = await response.data
         console.log('jobinfo result' , result)
         console.log(result)
          if(response.status==200){
            ...
            setIsProposalSent(result.isProposalSent)
          }else{
            
             if(response.status==400){
            //  setJobError("default") 
              if(result.errors.type=="not_connected"){
                setJobError('notLoggedIn')  
              }      
            }else if(response.status==404){
...
toast.error(LikedServerErrorMessage.title, {
  ...
  const fetchJobsForYou=async({lastCursorPromoted,promotedPrevDocs,lastCursorRegular,regularPrevDocs,setLastCursorPromoted,setPromotedPrevDocs,setLastCursorRegular,setRegularPrevDocs,setPromotedHasMore,setRegularHasMore,setFetchLoading,setJobs}:fetchJobsForYouInterface)=>{

    try{

      const response = await Jobsforyou(lastCursorPromoted,promotedPrevDocs,lastCursorRegular,regularPrevDocs)
      const result = await response.data;
      if(response.status==200){
        console.log('result',result)
        setLastCursorPromoted(result.promotedLastCursor)
        setPromotedPrevDocs((prev)=>[...prev,...result.promotedPrevIds])
        setLastCursorRegular(result.regularLastCursor)
        setRegularPrevDocs((prev)=>[...prev,...result.regularPrevIds]) 
        setPromotedHasMore(result.promotedHasMore)
        setRegularHasMore(result.regularHasMore)
        setJobs((prev)=>[...prev,...result.regularJobs,...result.promotedJobs])
        // setFetchLoading(false)
      }else if(response.status==404){
          // setFetchLoading(false)
          toast.error(errorFetchingJob404.title,{
            description:errorFetchingJob404.description
          })
          setRegularHasMore(false)
          setPromotedHasMore(false)
        }else {
        //  setFetchLoading(false) 
      ...
      console.log('Error At Use Hook Error FetchJobsForYou',err)
    }
    finally{
      setFetchLoading(false)
    }
