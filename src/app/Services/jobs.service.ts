...
export const fetchingJobs = async(
  ...
  const newjobUrl = process.env.JOB_SERVICE
      ...
      console.error('findlJobUrl',finalJobUrl)
   try{
      
    const response = await fetch(`${finalJobUrl}/Main/GET/mainjobs`,
       { headers:{
...
export const saveJob = async({jobId,isSaved})=>{
  try{
    const response = await jobInstance.post(`/job/saved`, 
...
export const Jobsforyou = async(lastCursorPromoted,promotedPrevDocs,lastCursorRegular,regularPrevDocs)=>{
    try{


      const response = await jobInstance.post('/foryou/jobs',{lastCursorPromoted,promotedPrevDocs,lastCursorRegular,regularPrevDocs})
      
      return response;


    }catch(err){
      console.log('error at  JobsFor you' , err)
      throw Error('Error Fetching Jobs For you')
    }
   }
